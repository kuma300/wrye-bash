...
class PageSelect(PageInstaller):
    ...
    def __init__(self, parent, bMany, title, desc, listItems, listDescs, listImages, defaultMap):
        ...
        if bMany:
            self.listOptions = list_box(kind=u'checklist')
    ...
    def OnNext(self):
        ...
        if self.parent.parser.choiceIdex < len(self.parent.parser.choices):
            ...
            self.parent.parser.choices.append(temp)
        self.parent.parser.PushFlow(u'Select', False, [u'SelectOne', u'SelectMany', u'Case', u'Default', u'EndSelect'], values=temp, hitCase=False)
...
class PageFinish(PageInstaller):
    ...
    def __init__(self, parent, subsList, plugin_list, plugin_renames, bAuto,
        ...
        # Sub-packages
        self.listSubs = balt.listBox(self, choices=subs, kind=u'checklist',
...
class PageVersions(PageInstaller):
    ...
    def __init__(self, parent, bGameOk, gameHave, gameNeed, bSEOk, seHave,
        ...
        PageInstaller.__init__(self, parent)
        bmp = [Image(bass.dirs[u'images'].join(u'error_cross_24.png').s)
                   .GetBitmap(),
               Image(bass.dirs[u'images'].join(u'checkmark_24.png').s)
                   .GetBitmap()]
...
class WryeParser(ScriptParser.Parser):
    ...
    def Begin(self, file):
        ...
        if file.exists() and file.isfile():
            try:
                with file.open(encoding=u'utf-8-sig') as script:
        ...
        while self.cLine < len(self.lines):
            ...
            try:
                ...
                return PageError(self.parent, _(u'Installer Wizard'),
                                 _(u'An error occurred in the wizard script:') + u'\n'
                                 + _(u'Line %s:\t%s') % (self.cLine, newline.strip(u'\n')) + u'\n'
                                 + _(u'Error:\t%s') % e)
            except Exception:
                o = StringIO.StringIO()
                o.write(_(u'An unhandled error occurred while parsing the wizard:') + u'\n'
                        + _(u'Line %s:\t%s') % (self.cLine, newline.strip(u'\n')) + u'\n\n')
    ...
    def _KeywordSelect(self, bMany, name, *args):
        ...
        for i in images:
            path = imageJoin(i)
            if not path.exists() and bass.dirs[u'mopy'].join(i).exists():
                path = bass.dirs[u'mopy'].join(i)
