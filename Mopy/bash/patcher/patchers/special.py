...
class _AListsMerger(AListPatcher):
    ...
    def __init__(self, p_name, p_file, p_sources, remove_empty, tag_choices):
        ...
        self.masterItems = defaultdict(set)
...
class _AListsMergerImpl(_AListsMerger, ListPatcher):
    """Common code from _AListsMerger subclasses - TODO(inf) Better name"""
        ...
        if sc_name in self.de_masters:
            for list_type in self._read_write_records:
                for de_list in getattr(modFile, list_type).getActiveRecords():
                    self.masterItems[sc_name] = set(self._get_entries(de_list))
        #--Relev/Delev setup
        ...
        for list_type in self._read_write_records:
            ...
            for new_list in new_lists.getActiveRecords():
                ...
                if not is_list_owner:
                    #--Relevs
                    new_list.re_plugins = items.copy() if is_relev else set()
                    #--Delevs: all items in masters minus current items
                    new_list.de_plugins = delevs = set()
                    if is_delev:
    ...
    def buildPatch(self, log, progress):
        ...
        for list_type, list_label in self._type_to_label.iteritems():
            ...
            for list_eid in sorted(removed_empty_sublists, key=string.lower):
                log(u'* ' + list_eid)
            log.setHeader(u'=== ' + _(u'Empty %s Sublists Removed') %
                          list_label)
            for list_eid in sorted(cleaned_lists, key=string.lower):
                log(u'* ' + list_eid)
...
class ListsMerger(_AListsMergerImpl):
...
#------------------------------------------------------------------------------
class FidListsMerger(_AListsMergerImpl):
    """Merges FormID lists."""
    scanOrder = 46
    editOrder = 46
