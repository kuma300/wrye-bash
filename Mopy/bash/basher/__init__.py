...
if sys.prefix not in set(os.environ['PATH'].split(u';')):
    os.environ['PATH'] += u';'+sys.prefix
...
class InstallersList(balt.UIList):
    ...
    def OnKeyUp(self,event):
        ...
        # Ctrl+C: Copy file(s) to clipboard
            ...
            sel = [bass.dirs[u'installers'].join(x).s for x in self.GetSelected()]
            balt.copyListToClipboard(sel)
...
class ScreensList(balt.UIList):
    ...
    def _order_by_number(self, items):
        if self.sort_column != u'File': return
        ...
        items.sort(key=keys.__getitem__,
                   reverse=self.colReverse.get(u'File', False))

    _sort_keys = {u'File'    : None,
                  u'Modified': lambda self, a: self.data_store[a][1],
                  u'Size'    : lambda self, a: self.data_store[a][2],
                 }
    #--Labels
    labels = OrderedDict([
        (u'File',     lambda self, p: p.s),
        (u'Modified', lambda self, p: format_date(self.data_store[p][1])),
        (u'Size',     lambda self, p: round_size(self.data_store[p][2])),
    ])
