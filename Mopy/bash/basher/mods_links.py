...
class Mods_OblivionVersion(CheckLink, EnabledLink):
    ...
    def Execute(self):
        ...
        if self.setProfile:
            bosh.saveInfos.profiles.setItem(bosh.saveInfos.localSave,u'vOblivion',self.key)
        Link.Frame.SetTitle()
...
class Mods_AutoGhost(BoolLink):
    ...
    _text, key = _(u'Auto-Ghost'), u'bash.mods.autoGhost'
