...
def parse():
    ...
    parser.add_argument(u'-d', u'--debug',
                        ...
                        help=u'Useful if bash is crashing on startup or if '
                             ...
                             u' (e.g. while developing or '
                             u'debugging).')
    parser.set_defaults(mode=0)
    parser.add_argument(u'-C', u'--Cbash-mode',
                        action=u'store_const',
                        const=2,
                        dest=u'mode',
                        help=u'enables CBash and uses CBash to build bashed '
                             u'patch.')
    parser.add_argument(u'-P', u'--Python-mode',
                        action=u'store_const',
                        const=1,
                        dest=u'mode',
                        help=u'disables CBash and uses python code to build '
                             u'bashed patch.')
    ...
    parser.add_argument(u'--uac',
                        action=u'store_true',
                        dest=u'uac',
                        help=u'always start in admin mode if UAC protection '
                             u'is detected.')
    parser.add_argument(u'--genHtml',
                        default=None,
...
_short_to_long = {
    u'-o': u'--oblivionPath', u'-p': u'--personalPath', u'-u': u'--userPath',
    u'-l': u'--localAppDataPath', u'-b': u'--backup', u'-r': u'--restore',
    u'-q': u'--quiet-quit', u'-f': u'--filename', u'-d': u'--debug',
    u'-C': u'--Cbash-mode', u'-P': u'--Python-mode', u'-L': u'--Language',
}
