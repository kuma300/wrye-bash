...
from ..bolt import LowerDict, CIstr, deprint, GPath, DefaultLowerDict, \
    decode, getbestencoding, OrderedLowerDict
...
class IniFile(AFile):
    ...
    def remove_section(self, target_section, do_backup=False):
        ...
        """Removes a section and all its contents from the INI file, optionally
        making a backup of the file. Note that this will only remove the first
        matching section. If you want to remove multiple, you will have to call
        this in a loop and check if the section still exists after each
        iteration."""
...
class DefaultIniFile(IniFile):
    ...
    def __init__(self, default_ini_name, settings_dict=None):
        # we don't call AFile#__init__ to avoid stat'ing the non existing file
        if settings_dict is None:
            settings_dict = self.__empty
