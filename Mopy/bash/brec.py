...
class MelStructA(MelStructs):
    ...
    def loadData(self, record, ins, sub_type, size_, readId):
        ...
        # Note for py3: we want integer division here! Thank you!
...
class MelTruncatedStruct(MelStruct):
    ...
    def __init__(self, sub_sig, sub_fmt, *elements, **kwargs):
        ...
        :param kwargs: Must contain an old_versions keyword argument, which
            specifies the older formats that are supported by this struct. May
    ...
    def loadData(self, record, ins, sub_type, size_, readId):
        ...
        # Actually unpack the struct, then pad it with defaults if it's an
        # older, truncated version
        unpacked_val = ins.unpack(target_fmt, size_, readId)
        unpacked_val += self.defaults[len(unpacked_val):]
...
class MelSet(object):
    ...
    def getDefault(self, attr):
        ...
        return self.defaulters[attr].getDefault(attr)
